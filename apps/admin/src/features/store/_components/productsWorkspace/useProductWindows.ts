...
type ListWindow = { key: 'list'; label: 'Store'; isDirty?: never };
type NewWindow = { key: string; label: 'New Product'; isNew: true; isDirty: boolean };
type ExistingWindow = { key: string; label: string; id: string; isDirty: boolean };

export type ProductWindowEntry = ListWindow | NewWindow | ExistingWindow;

const LIST_WINDOW: ListWindow = { key: 'list', label: 'Store' };

function getDraftKey() {
  return crypto.randomUUID();
}

export function useProductWindows() {
  const [windows, setWindows] = useState<ProductWindowEntry[]>([LIST_WINDOW]);
  ...
  const openNew = useCallback((background = false) => {
    ...
    setWindows((prev) => [...prev, { key, label: 'New Product', isNew: true, isDirty: false } satisfies NewWindow]);
  ...
  const setWindowDirty = useCallback((key: string, isDirty: boolean) => {
    setWindows((prev) => {
      ...
      const next = prev.map((windowEntry): ProductWindowEntry => {
        if (windowEntry.key === key && windowEntry.key !== 'list') {
          ...
          changed = true;
          return { ...windowEntry, isDirty } as ProductWindowEntry;
        }
    ...
    setWindows((prev) => {
      ...
      if (existingIndex >= 0) {
        return prev
          ...
          .map((windowEntry): ProductWindowEntry => {
            if (windowEntry.key === id && windowEntry.key !== 'list') {
              return { ...windowEntry, label, isDirty: false } as ProductWindowEntry;
            }
      ...
      return prev.map((windowEntry): ProductWindowEntry => {
    ...
    setWindows((prev) => {
      ...
      const next = prev.map((windowEntry): ProductWindowEntry => {
        if (windowEntry.key === key && windowEntry.key !== 'list') {
          ...
          changed = true;
          return { ...windowEntry, label } as ProductWindowEntry;
        }
